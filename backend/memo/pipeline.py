...
#     if case_facts:
#         sem2 = detect_crime_type_semantic(case_facts[:9000])
#         if sem2:
...
#     ], temperature=0.15, max_tokens=15000)
...
# ما تكون مكتوبة نصاً صريحاً جوه الكود.
#
# ✏️ تعديل التوليد المقسّم (جديد): التوليد الأولي بقى 3 نداءات مركّزة
# (وقائع / دفوع / طلبات) بدل نداء واحد ضخم — بالظبط زي contract_pipeline.py
# بيولّد كل بند لوحده. جولات التصحيح (self-correction) لسه بتشتغل بنفس
# المنطق الأصلي (نداء واحد على المذكرة الكاملة).
...
def llm_text(messages: list[dict], temperature: float = 0.2, max_tokens: int = 3000,
             model: str | None = None, retries: int = 1) -> str:
    """نداء موحّد لأي completion من الـ LLM، بيتأكد إن الرد فيه نص فعلي قبل
    ما يرجّعه. أحيانًا OpenRouter بيرجّع content=None (تعطل مؤقت من المزوّد،
    أو رفض ضمني) — من غير الفحص ده، أي كود بيستخدم .strip() على الرد بيكسر
    ...
    ⚠️ num_ctx=8192: رقم مؤكد إنه بيتحمّل بالكامل على GPU بذاكرة 16GB (تأكيد
    عبر nvidia-smi/ollama ps — 99% GPU، صفر offload للـ CPU). لو غيّرتي جهاز
    أو كارت شاشة، أعيدي التأكد قبل ما تكبّري الرقم ده."""
    _ensure_pipeline()
    if not _llm:
        raise RuntimeError("LLM مش متاح — تأكد من OLLAMA_BASE_URL في ملف .env")
